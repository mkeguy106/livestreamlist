...
**Playback profiles** (`InlineVideo.jsx`, round 4): three cushioned mpegts.js configs, chosen ONCE at player-creation time from `variant` + `muted` — Focus → `FOCUS_CONFIG` (maxLatency 3, minRemain 1); muted column → `COLUMN_MUTED_CONFIG` (6 / 2); audible column → `COLUMN_UNMUTED_CONFIG` (4 / 1.5). All share `BASE_CONFIG`: `enableStashBuffer: true`, `liveBufferLatencyChasing: true`, and `CLEANUP_CONFIG` (`autoCleanupMaxBackwardDuration: 60 / Min: 30`). Jank correlation drove this: the prior round's zero-stash / 0.5 s-min-remain profile + 30/15 cleanup moved the stutter *onset* to ~20 s (matching cleanup cadence) — WebKitGTK blocks MSE appends while `SourceBuffer.remove()` runs, and the tiny-append zero-stash profile starved into a latency-chase stutter. Stash-on + ≥1 s min-remain cushions the remove() pauses; 60/30 halves remove() frequency. Deliberate tradeoff: ~1–3 s more end-to-end latency.
...
| WebKitGTK crashes with `Error 71 (Protocol error)` on NVIDIA + KDE Wayland | Historically `WEBKIT_DISABLE_DMABUF_RENDERER=1` was set unconditionally in `lib.rs::apply_linux_webkit_workarounds`. **As of round 4 it is NOT set by default** — the `video.dmabuf_renderer` setting defaults **true** (renderer ON) because the inline-video spike (`docs/superpowers/spikes/2026-07-07-inline-video-playback-spike.md`) measured ~4x cheaper video painting with it on and no crash on WebKit 2.52, and video decode needs the headroom. To restore the old workaround: set `video.dmabuf_renderer = false` (Preferences → Video → "Use GPU dmabuf renderer") or export `WEBKIT_DISABLE_DMABUF_RENDERER=1` yourself — a user export always wins (`set_if_unset` never overrides; when the setting is true we simply don't set the var). If other WebKit weirdness hits, try `GDK_BACKEND=x11` |
