...
| `mpv_set_visible` | `uniqueKey, visible: bool` | Hide/show one mpv surface (used for hover-occlusion and modal occlusion). The surface is created shown (GTK3 can't allocate hidden widgets — see the pitfalls row), so a not-yet-playing surface is a brief black rect, not the poster |
...
- **Focus** — single-stream reader mode. Tab strip of all channels across the top; split 60/40 with mpv-backed video (Linux; mpegts.js elsewhere) / chat.
