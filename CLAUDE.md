...
└── src/
    ...
    ├── mpv.rs               # MpvProcess spawn (--wid, per-OS default vo — Linux x11/--hwdec=auto-copy, Windows gpu — LSL_MPV_VO overrides), per-OS JSON IPC (Unix socket / Windows named pipe), monitor task, Windows Job Object PDEATHSIG analog (`mod job`)
    ├── chat/
    ...
    └── video/
        ├── mod.rs           # VideoManager — session map, spawn/stop/linger/reap, video:status emits; start_direct() hands the mpv backend (Linux/Windows) the raw streamlink URL, bypassing the CORS passthrough
...
| `video_backend` | — | Which inline-video backend this build/platform uses: `"mpv"` on Linux and Windows (slice C — Windows ships runtime-unverified; `LSL_MPV_VO` env var overrides the vo in the field), `"mpegts"` on macOS. A hardcoded target-OS check, not a user setting. Cached module-wide by `useVideoBackend`; `VideoPanel` renders `<MpvVideo>` or `<InlineVideo>` accordingly |
| `mpv_mount` | `uniqueKey, x, y, width, height, quality?, muted, volume` | Start (or resume) an mpv-backed session and mount its surface (`GtkDrawingArea` on Linux, a child HWND on Windows) at the given physical-pixel rect; idempotent on an already-mounted key (**resize-only** — will NOT restart mpv against a new URL, so quality switches must `mpv_unmount` first). Emits `mpv:status:{uniqueKey}` itself for the `starting`/`cap`/`error` states reached before the monitor task takes over. Linux + Windows; stubbed to an `Err` on other targets (macOS) and in smoke/test builds |
| `mpv_bounds` | `uniqueKey, x, y, width, height` | Reflow an existing mpv surface (e.g. on layout change) |
| `mpv_set_visible` | `uniqueKey, visible: bool` | Hide/show one mpv surface (used for hover-occlusion and modal occlusion) (hover path: Columns only since the Focus redesign). The surface is created shown (GTK3 can't allocate hidden widgets — see the pitfalls row), so a not-yet-playing surface is a brief black rect, not the poster |
| `mpv_unmount` | `uniqueKey` | Drop the mpv child; `MpvChild::Drop` SIGKILLs the process and destroys the surface (`GtkDrawingArea` on Linux, the host HWND on Windows). The monitor task's socket-EOF then reports `consumer_dropped` to `VideoManager`, starting linger |
...
- `<EmbedLayer modalOpen={anyDialogOpen}>` is mounted once at App.jsx scope. It's the **only** component that calls `embed_*` IPC, and likewise the sole caller of the `mpv_*` geometry/lifecycle IPC (`mpv_mount`/`mpv_bounds`/`mpv_set_visible`/`mpv_unmount` — the live volume/mute controls in `MpvVideo.jsx` go over `mpv_set_volume`/`mpv_set_muted` directly). Owns a `Map<EmbedKey, { refs: Map<slotId, {ref, active}> }>` registry.
...
**Popout hand-off**: `InlineVideo.popout()` stops the inline session and launches mpv, holding a `popout` poster ("Starting external player…") until `usePlayerState` reports the external player live — then the column unmounts and Focus shows a `popped` resting state with a "Play inline" button. `MpvVideo.jsx` mirrors the identical mechanism (popout/popped phases, 10 s safety net if the external player never starts, retry-as-"Play inline") and is what runs for Focus on Linux and Windows.

**mpv backend (slice A)**: on Linux (and, since slice C, Windows), inline video is played by an embedded `mpv` process rather than mpegts.js-over-MSE — see `mpv.rs` and the `embed.rs` `ChildInner::Mpv` variant. Backend selection is a build-time/OS check (`video_backend` IPC, `cfg!(any(target_os = "linux", target_os = "windows"))`); `VideoPanel.jsx` reads it via `useVideoBackend()` and renders `<MpvVideo>` (mpv) or the pre-existing `<InlineVideo>` (mpegts.js) accordingly, so macOS and the smoke/test builds are unaffected. Slice C added the Windows twin: a `CreateWindowExW` "STATIC"-class child HWND host (`embed.rs::surface_windows`), named-pipe JSON IPC, a KILL_ON_JOB_CLOSE Job Object as the PDEATHSIG analog, and `--vo=gpu` — **runtime-unverified** (no Windows test hardware; the blocking `rust-windows` CI job is the compile gate). `EmbedLayer.jsx` branches every mount/bounds/visibility/unmount call on the slot's `backend` prop (`mpv_*` IPC vs the existing `embed_*` IPC) and adds two mpv-only mechanisms: **`occludeKey`**, a per-key hide (distinct from the global `useEmbedOcclusion`) so hovering one column's video to reveal its DOM control strip (mute/volume/quality/pop-out/stop) doesn't blank every other video or chat embed — only the global modal path still hides everything; and **`remountKey`**, an explicit unmount-then-reflow used for both quality switches (mount is idempotent-resize-only, so changing quality requires killing and respawning mpv against a new streamlink URL) and the Retry button after a stream death (the monitor task's socket-EOF path unmounts the mpv child on the Rust side, but `EmbedLayer`'s client-side `mountedKeys` set is unaware, so a plain reflow would take the "already mounted" branch and silently no-op forever — `remountKey` unmounts first, a safe no-op Rust-side, before reflowing into a genuine fresh `mpv_mount`). mpv rides the same `VideoManager` linger/cap/generation machinery as the mpegts backend: `embed::mount_mpv` calls `consumer_connected` before the monitor task can observe an exit, and the monitor's socket-EOF handler calls `consumer_dropped`, so an mpv session lingers (streamlink stays warm) exactly like a browser-passthrough one. App-exit reap kills both process families: `RunEvent::Exit` calls `VideoManager::stop_all()` (streamlink) and, on Linux/Windows, `EmbedHost::stop_all_mpv()` (mpv) — `MpvProcess` also sets `PR_SET_PDEATHSIG=SIGKILL` on Unix / assigns a KILL_ON_JOB_CLOSE Job Object on Windows at spawn so an abrupt parent death (crash, `SIGKILL`) can't orphan mpv even when neither `Drop` nor `RunEvent::Exit` runs.

**mpv backend (slice B)**: Focus now routes through `VideoPanel` too (`Focus.jsx`'s `FeaturedStream` renders `<VideoPanel variant="focus">`, `key`-ed on `channel.unique_key` so a tab switch forces a clean remount rather than bleeding mount-seeded mute/volume state across channels) — the mpv path is no longer Columns-only. Mount-arg resolution for both variants is centralized in `src/utils/mpvMountArgs.js` (`resolveMpvQuality` / `mpvQualityLabel` / `initialMpvMuted`, mirroring `InlineVideo.jsx`'s mpegts variant matrix exactly): Focus always requests `quality: null` so Rust's own per-channel → `default_quality` resolution applies ("best" out of the box, matching mpegts's round-5 behavior) while the label ref shows what Rust *will* resolve to; Focus's `initialMpvMuted` ignores any persisted per-channel `muted` and always returns `false` — the single featured stream always starts audible, whereas Columns still respects persisted mute / `autoplay_unmuted`. `mpvMountArgs.js` has module-level DEV asserts covering both branches. A `starting`/`playing`/`cap`/`ended`/`error` state machine driven entirely by `mpv:status:{key}` events now bounds `error` with an auto-retry ladder before surfacing the error chip: `MAX_AUTO_RETRIES = 3` at `AUTO_RETRY_BACKOFF_MS = [1000, 2500, 5000]`, deliberately slower than `InlineVideo.jsx`'s mpegts ladder (500/1000/2000) because a retry inside `VideoManager`'s ≤5 s sweep window would resume the corpse streamlink port of the stream that just died and fail again — the schedule is chosen so the third attempt lands past that window. Each retry keeps the spinner (`starting` phase, no error flash) and calls `layer.remountKey(channelKey)`; the budget resets to 0 on a `playing` event (recovery) and on a fresh channel mount. `EmbedLayer`'s context object (`register`/`unregister`/`updateActive`/`reflowKey`/`pushOverlay`/`occludeKey`/`remountKey`, memoized together as `ctx`) is now identity-stable across `modalOpen`/occlusion-count changes: `reflowKey` reads `hidden` through a ref (`hiddenRef`) instead of closing over it directly, so its `useCallback` has an empty dep array and the downstream `ctx` `useMemo` never produces a new object on a modal open/close. Previously `reflowKey` depended on `hidden`, so every Preferences/dialog toggle changed `ctx` identity, which every `EmbedSlot`'s register-effect treats as "layer changed" — cleanup (`unregister`) then setup (`register`) fired for every mounted slot, and for `backend: 'mpv'` slots `unregister` tears down to `mpv_unmount` (killing the mpv child) while `register` reflows into a fresh `mpv_mount` — so opening Preferences with videos playing looked like every video restarting from a black frame. The fix keeps `hidden`'s effect purely reactive: the existing "re-apply visibility when modal/overlay state toggles" `useEffect([hidden])` still walks `mountedKeys` and calls `mpvSetVisible`/`embedSetVisible` directly, which hides/shows without touching mount state at all. The Focus redesign (spec `docs/superpowers/specs/2026-07-10-focus-redesign-design.md`) later replaced the focus variant's hover-occlusion controls with a persistent bar below the video rect — `occludeKey` is now a **column-only** mechanism; the focus variant registers no hover handlers and never occludes its surface.
...
| GTK3 refuses to size-allocate invisible widgets — a created-hidden mpv surface keeps the default 1×1@(-1,-1) GdkWindow forever | `gtk_widget_size_allocate` early-returns for `!visible` non-toplevel widgets, and containers skip hidden children in their allocation pass, so a "create hidden, show when mpv confirms playback" surface renders mpv into ONE PIXEL with no error anywhere (found in the slice-A live smoke: decode counters healthy, nvdec active, nothing on screen). `embed.rs::mount_mpv` therefore calls `area.show()` **before** `area.realize()` (exactly like the spike) and accepts a sub-second black rectangle between mpv spawn and first frame — the DOM poster still covers the multi-second streamlink startup because the surface doesn't exist until `mpv_mount`'s GTK step. Related one-shot-event lesson in the same smoke: the monitor emits `playing` once, so a webview reload (dev HMR, crash recovery) would spin on the startup poster forever — `mount_mpv`'s idempotent branch returns `already_ready` and `mpv_mount` re-emits `playing` for exactly that case |
| The Windows mpv path (slice C) has never run on real hardware — the vo (`gpu`), the WS_EX_TRANSPARENT hit-test pass-through, and named-pipe IPC are all best-effort ports of the verified Linux mechanisms | If a Windows user reports a black video rect or dead hover controls, first try `LSL_MPV_VO=d3d11` (then `gpu-next`) via env var; the DOM poster/controls layer is unaffected either way. The `rust-windows` CI job only proves compilation. Most likely first failure on hardware: Columns hover-controls — mpv creates its own opaque child window INSIDE our WS_EX_TRANSPARENT host, and that inner window does not inherit hit-test transparency (`--input-cursor-passthrough` governs mpv's input handling, not win32 hit-testing); candidate fix is applying WS_EX_TRANSPARENT to the inner window post-spawn. Focus is unaffected (its control bar sits below the video rect) |
| A `useCallback`/`useMemo`'d React context value that closes over reactive state gets a new identity on every render of that state, cascading into every consumer's register-effects | `EmbedLayer.jsx`'s `ctx` (`register`/`unregister`/.../`remountKey`) is provided to every `EmbedSlot` in the tree. Before the fix, `reflowKey` closed over `hidden` directly and listed it in its own `useCallback` deps; every App-level modal open/close changed `hidden`, which gave `reflowKey` — and therefore the whole `ctx` `useMemo` — a new identity. `EmbedSlot`'s register-effect (see the row above) treats "layer changed" as reason to `unregister` then `register`, and for `backend: 'mpv'` slots that means `mpv_unmount` (kills the mpv child) immediately followed by a fresh `mpv_mount` — every playing video visibly restarted on every Preferences/dialog open. Fix: read reactive state consumers need only for a one-off decision (not for re-triggering the effect) through a ref (`hiddenRef`, updated by its own small `useEffect([hidden])`) so the callback's dep array can stay `[]`/stable, and let a *separate* effect apply the reactive side-effect directly (`mpvSetVisible`/`embedSetVisible` on `mountedKeys`) without touching the context identity at all. General lesson: before adding a value to a memoized context to a dependency array, check whether every consumer effect keyed on that context's identity actually wants to re-run, or would rather read the value live through a ref |
| A component whose React hook count differs between renders of the same instance (an early return between hook calls, e.g. per-platform branches) throws `Rendered more hooks than during the previous render` — React 18 unmounts the WHOLE ROOT, not just that component. The app becomes a black window with a healthy process and zero terminal output (WebKit's console is invisible in `tauri:dev`) | This was the "Focus + Chaturbate black window" bug — never WebKit, never the embeds. Focus renders one unkeyed `<ChatView channelKey={featured}>`; switching the featured channel between an embed platform (YT/CB — 2 hooks) and an IRC platform (Twitch/Kick — dozens of hooks) re-rendered the *same instance* with a different hook count. Command was structurally immune — its chat tabs keep one `ChatView` mounted per channel, so an instance never gets re-purposed across platforms. Fix: split per-platform/per-variant render paths into separate components dispatched by a zero-hook parent — `ChatView` is now a dispatcher calling no hooks itself, returning `<EmbedChatView>` (YT/CB) or `<IrcChatView>` (everything else); a platform change swaps the component *type* (clean remount) instead of re-rendering one instance. Full diagnosis trail: `docs/superpowers/diagnosis/2026-07-11-cb-black-window.md` |
