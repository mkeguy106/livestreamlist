...
- [ ] **Hyperlink support in chat — clickable URLs open in browser** — detect `http://` / `https://` URLs in chat-message text and render them as clickable links that fire `open_in_browser` (existing IPC) on click. Detection runs in the same pipeline as emote scanning so it composes naturally: in Rust `chat/emotes.rs::scan_message` already produces `EmoteRange[]`; add a peer scan that produces `LinkRange[]` (start/end byte offsets + the URL string), thread it onto `ChatMessage`, and have `EmoteText.jsx` interleave `<a>` segments alongside the existing emote/text segments when rendering. Frontend click handler calls `invoke('open_in_browser', { url })` rather than letting the browser's default `<a href>` navigation happen — the chat lives in a webview, so a default click would replace the chat with the linked page. Visual treatment: zinc-300 underline on hover, no underline at rest (matches the chat's existing minimal aesthetic). Mind: URLs split across multiple lines via `\r\n` in IRC are already a single message at the chat layer. Twitch's auto-link-mod often inserts zero-width characters into URLs to defeat link previews — the regex should tolerate that (use a permissive matcher then validate with `url::Url::parse` before adding to `LinkRange`). Out of scope for v1: link previews, OG-tag fetching, embedded thumbnails, internal navigation (e.g., clicking a `twitch.tv/shroud` URL opening the channel inside the app). All of those are follow-ups; v1 just opens externally.
