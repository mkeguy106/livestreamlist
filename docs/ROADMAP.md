...
- [ ] **Spellcheck + autocorrect** — chat composer gets red wavy underlines on misspellings; high-confidence misspellings (single suggestion, Damerau-Levenshtein ≤ 1, or apostrophe expansion like `dont→don't`) auto-correct with a chiclet-bordered green pill highlight on the corrected word that fades over ~3 s. Right-click on a misspelled word for top-5 suggestions + "Add to dictionary" + "Ignore in this message". Esc within 5 s reverts the most recent autocorrect. Skip-token rules for `@mentions`, URLs (incl. bare `domain.tld/path`), emote codes (per-channel emote list + `:colon-form:`), and all-caps shorthand. Default-on toggles in Preferences Chat tab; manual language selection from installed hunspell dicts (no fragile per-message auto-detect on chat-length text). Personal dictionary persists at `~/.config/livestreamlist/personal_dict.json`. **Includes a fix for the Qt app's "autocorrect re-fires when editing mid-word" bug** — adds a cursor-position guard so autocorrect only fires when the caret is OUTSIDE the word being checked. Spec: `docs/superpowers/specs/2026-05-01-spellcheck-design.md`. Decomposed into 5 PRs:
  ...
  - [ ] **PR 3 — Autocorrect + green pill + bug fix + Esc-to-undo** — port Qt's confidence rules (apostrophe expansions, single suggestion, Damerau-Levenshtein ≤ 1) plus the cursor-position guard. Chiclet-bordered green pill on autocorrected words (rgba(60,200,60,0.12) fill + rgba(60,200,60,0.6) 1 px border, 3 px border-radius), CSS fade over 3 s. Esc within 5 s reverts the autocorrect. Recently-corrected memory per Composer session. Includes the regression test for the reported Qt bug.
