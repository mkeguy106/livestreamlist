...
- [ ] **Headless IPC smoke-test harness** — agent-driven implementation pipelines (subagent-driven-development for the spellcheck PR series, future automated CI smoke jobs) currently hit a wall when an implementation step requires verifying that a `#[tauri::command]` actually works end-to-end through the IPC layer. Cargo unit tests construct the underlying types directly and bypass `tauri::generate_handler!`, so they don't catch IPC marshalling bugs (wrong arg names in `invoke()` payloads, missing entries in `generate_handler![...]`, serde-skip mismatches between Rust struct and the JS-side shape). Today the workaround is "ask the human to launch the app, paste invocations into devtools, paste results back" — clean for a one-off but a tax on every agent-driven PR. Options: (a) a hidden `--smoke <jsonl>` CLI flag on the Tauri binary that consumes invoke calls on stdin and emits results on stdout; (b) `tauri-driver` (WebDriver-compatible) for full UI scripting of clicks + console scripts; (c) a `cargo run --bin smoke -- <command> '<json args>'` thin wrapper that constructs an `AppHandle` minimally enough to dispatch one command. (a) is cheapest and unblocks the most. Driver case: PR series implementing spellcheck (PRs #92-#96 once landed) and beyond — every IPC-touching PR after this benefits.
