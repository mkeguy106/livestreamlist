...
- [ ] **YouTube live-status: switch to scrape-first (Qt parity)** — current `platforms/youtube.rs::fetch_live` runs `yt-dlp` first and layers HTML scrapes on top for multi-stream detection. yt-dlp itself fires 3-5 internal YouTube requests per invocation (player config / formats / etc.), and at YT_CONCURRENCY × 60s refresh × N channels this triggers IP throttling that Qt doesn't hit. Qt's `api/youtube.py::get_livestream` scrapes `youtube.com/channel/{id}/live` first and only falls back to yt-dlp when the scrape fails. Refactor: add `fetch_live_via_scrape(http, channel_id)` that GETs `/live`, parses `ytInitialPlayerResponse` for `videoDetails` (live? title? viewers? thumbnail?) and `microformat.playerMicroformatRenderer.liveBroadcastDetails.startTimestamp`, and returns a `YouTubeLive`. Wire it as the primary path; keep current `fetch_primary_via_ytdlp` as fallback when scrape returns no parseable player response. Multi-stream `/streams` + per-video `/watch` scrapes stay (already cheap relative to yt-dlp). Net effect: ~3-5× reduction in YT request volume per refresh, IP throttle stops firing, and the YT handle-detection fetch from PR #41 stops getting blocked as collateral damage.
...
- [ ] **Global hover + focus audit** — sweep every interactive element and normalize hover / focus-visible styling against `tokens.css`. Known offenders: chat emotes, the account chip in the titlebar, the preferences icon, add-channel and refresh buttons, layout-switcher dots, column header controls. Currently several of them fall through to WebKit's native cursor / outline, breaking the visual identity.
  - Themed tooltip first pass (PR #39) — extracted `src/components/Tooltip.jsx` from the inline popover that lived in `Command.jsx`'s `IconBtn` and applied it to the titlebar layout-dots, gear button, login chiclet, and sidebar filter/sort/hide/refresh controls. Native `title=""` rendering by WebKitGTK on those was the most jarring break of the zinc design. Still TODO under this same audit: chat reply-banner click hint, `SocialsBanner` URL tooltips, `TitleBanner` / `Focus.jsx` channel-title tooltips (need a `whiteSpace: 'normal'` wrap variant), `Composer` popout button, `PreferencesDialog` browser-cookie buttons. Also pending: focus-visible parity (currently hover-only).
